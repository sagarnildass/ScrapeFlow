...
import { Workflow, WorkflowExecutionStatus } from "@/types/workflow"; // use this import instead of @prisma/client
        ...
        <div className="flex items-center justify-end space-x-3">
          ...
          <div>
            <h3 className="text-base font-bold text-muted-foreground flex items-center">
              <TooltipWrapper content={workflow.description || "No description"}>
                <Link
...
function ScheduleSection({
  ...
  return (
    <div className="flex items-center gap-2">
      <CornerDownRight className="h-4 w-4 test-muted-foreground" />
      <SchedulerDialog
        workflowId={workflowId}
        cron={cron}
        key={`${cron}-${workflowId}`}
      />
      <MoveRightIcon className="h-4 w-4 test-muted-foreground" />
...
function LastRunDetails({ workflow }: { workflow: Workflow }) {
  ...
  const nextSchedule = nextRunAt ? format(nextRunAt, "yyyy-MM-dd HH:mm") : null;
